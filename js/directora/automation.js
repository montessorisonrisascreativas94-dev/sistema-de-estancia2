/**
 * ?? Colegio Montessori Sonrisas Creativas � M�dulo de Automatizaci�n Inteligente
 * Detecta anomal�as, genera alertas y automatiza an�lisis para la directora.
 */
...
export const AutomationModule = {
  ...
  // -- 1. DETECCI�N DE RIESGO DE RETIRO -------------------------------------
  ...
  async getAtRiskStudents() {
    try {
      ...
      for (const [sid, info] of Object.entries(byStudent)) {
        const sorted = info.dates.sort((a, b) => b.localeCompare(a)); // m�s reciente primero
        if (sorted.length < 3) continue;

        // Verificar si las �ltimas 3 son consecutivas
        let consecutive = 1;
  ...
  // -- 2. SEM�FORO ACAD�MICO -------------------------------------------------
  // Detecta si m�s del 30% del aula est� por debajo del promedio m�nimo
  ...
  // Compara asistencia de hoy vs promedio de los �ltimos 7 d�as
  async getAttendanceAlert() {
    try {
      ...
      // Calcular promedio semanal por d�a
      const byDay = {};
  ...
  async renderSmartWidgets(containerId = 'smartAlertsContainer') {
    ...
    try {
      ...
      if (attAlert) {
        ...
        widgets.push(`
          <div class="bg-${color}-50 border border-${color}-200 rounded-2xl p-4 flex items-start gap-3">
            <div class="w-9 h-9 bg-${color}-100 rounded-xl flex items-center justify-center shrink-0 text-lg">??</div>
            <div class="min-w-0">
      ...
      // Widget: Sem�foro acad�mico
      if (academicAlerts.length > 0) {
        widgets.push(`
          <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4">
            <div class="flex items-center gap-2 mb-3">
              <span class="text-lg">??</span>
              <p class="text-xs font-black text-amber-800 uppercase tracking-wide">Alerta Acad�mica</p>
            </div>
            ${academicAlerts.slice(0, 2).map(a => `
              <div class="bg-white rounded-xl px-3 py-2 border border-amber-100 mb-2">
                ...
                <p class="text-[10px] text-amber-700">${a.belowMinCount} de ${a.totalStudents} estudiantes (${a.percentage}%) por debajo del m�nimo � Promedio: ${a.globalAvg}</p>
              </div>`).join('')}
          </div>`);
      }

      // Widget: Pr�ximos vencimientos
      if (upcomingDues.length > 0) {
        widgets.push(`
          <div class="bg-blue-50 border border-blue-200 rounded-2xl p-4">
            <div class="flex items-center gap-2 mb-3">
              <span class="text-lg">??</span>
              <p class="text-xs font-black text-blue-800 uppercase tracking-wide">Pr�ximos Vencimientos</p>
            </div>
              ...
              ${upcomingDues.slice(0, 3).map(p => `
                <div class="flex items-center justify-between bg-white rounded-xl px-3 py-2 border border-blue-100">
                  ...
                  <div class="text-right">
                    ...
                    <p class="text-[9px] font-bold ${p.daysLeft <= 1 ? 'text-rose-600' : 'text-slate-400'}">${p.daysLeft === 0 ? 'Hoy' : p.daysLeft === 1 ? 'Ma�ana' : `En ${p.daysLeft} d�as`}</p>
                  </div>
                </div>`).join('')}
            </div>
          </div>`);
