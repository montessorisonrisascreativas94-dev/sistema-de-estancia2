...
export const PaymentsModule = {
      ...
      if (monthKey > maxVisibleMonthKey) {
        ...
        tbody.innerHTML = `<tr><td colspan="8" class="text-center py-16">
          <div class="flex flex-col items-center gap-3">
            <div class="w-14 h-14 bg-[#E8F2FF] rounded-full flex items-center justify-center text-2xl">📅</div>
  ...
  downloadInvoice(id) {
    ...
    else Helpers.toast('No se encontr� el registro', 'warning');
  },

  // -- Modal de exportaci�n batch ------------------------------------
  _openExportModal() {
    const list = AppState.get('paymentsData') || [];
    ...
    window.openGlobalModal(
      '<div class="bg-gradient-to-r from-violet-600 to-purple-600 text-white p-6 rounded-t-3xl flex items-center gap-3">' +
        ...
        '<div><h3 class="text-xl font-black">Exportar Facturas</h3><p class="text-xs text-white/70 font-bold uppercase tracking-widest">Descarga electr�nica CSV</p></div>' +
      '</div>' +
      '<div class="p-6 space-y-2">' +
        ...
        row('review',  'En revisi�n',           '#2563eb', counts.review) +
