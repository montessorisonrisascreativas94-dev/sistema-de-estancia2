...
export const ChatModule = {
  ...
  async _loadContacts() {
    ...
    if (error) throw error;
      ...
      this._allContacts = (users || []).map(u => {
        ...
        // Para padres: mostrar nombre del estudiante como t�tulo principal
        // y nombre del padre como subt�tulo
  ...
  async _loadMessages() {
    ...
    try {
      // Reset paginaci�n al abrir un chat nuevo
      ...
      try {
        ...
        // get_direct_messages puede no existir a�n � mostrar chat vac�o
        messages = [];
        conversationId = null;
      }
      ...
      container.innerHTML = '';
      if (!messages.length) {
        container.innerHTML = '<div class="flex-1 flex flex-col items-center justify-center text-slate-400 opacity-60 gap-2"><i data-lucide="message-circle" class="w-10 h-10 text-blue-300"></i><p class="text-sm">Inicia la conversaci�n</p></div>';
  ...
  async sendMessage() {
    ...
    try {
      ...
      sendPush({ user_id: this._activeContactId, title: 'Nuevo mensaje de Direcci�n', message: text, type: 'chat' }).catch(() => {});
    } catch (e) {
  ...
  _subscribeRealtime() {
    ...
    const input = document.getElementById('chatMessageInput');
    const user = { name: 'Direcci�n' }; // O obtener de profiles
    let typingTimeout;
