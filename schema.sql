...
-- Tabla de AÑOS ESCOLARES (base de toda la arquitectura)
...
-- Tabla de PLANES DE PAGO (definidos por año escolar y nivel)
...
CREATE TABLE IF NOT EXISTS public.plan_installments (
  ...
  month_number         int NOT NULL, -- Número de cuota
  month_name          text NOT NULL, -- Ej: "Agosto", "Septiembre"
  amount              numeric(10,2) NOT NULL,
  due_day             int NOT NULL DEFAULT 5, -- Día de vencimiento
  due_month_offset  int NOT NULL DEFAULT 0, -- Offset desde inicio del año escolar (0 = agosto, 1=septiembre, etc.)
  is_registration   boolean DEFAULT false,
  UNIQUE(payment_plan_id, type, month_number)
);

-- Tabla de INSCRIPCIONES DE ESTUDIANTES POR AÑO ESCOLAR
...
-- Tabla de CARGOS INDIVIDUALES DEL ESTUDIANTE (el corazón financiero)
...
-- Agregar FK a student_charges solo si la tabla ya existe (ciclo académico)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Índices para plan_installments
...
-- Índices para student_enrollments
...
-- Índice para payments (solo si la columna existe)
DO $$
BEGIN
...
-- ============================================================
-- 9. MOTOR DE PLANES DE PAGO: Generar cargos automáticos
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.student_charges(
      ...
      CASE WHEN v_installment.is_registration THEN 'Inscripción ' || v_school_year.name ELSE 'Colegiatura ' || v_installment.month_name || ' ' || v_school_year.name END,
...
-- Trigger para generar cargos automáticamente al inscribir un estudiante
...
BEGIN
  ...
  IF NOT FOUND THEN
    BEGIN
      SELECT * INTO v_staff FROM public.profiles WHERE id = p_code::uuid AND role IN ('maestra','asistente','directora','admin') LIMIT 1;
...
CREATE POLICY "profiles_select" ON public.profiles FOR SELECT
  USING (auth.uid() = id OR COALESCE(get_my_role(), '') IN ('directora','asistente','admin'));
...
CREATE POLICY "attendance_requests_all" ON public.attendance_requests FOR ALL
  USING (COALESCE(get_my_role(), '') IN ('directora','asistente','admin','maestra') OR
...
CREATE POLICY "task_evidences_all" ON public.task_evidences FOR ALL
  USING (COALESCE(get_my_role(), '') IN ('directora','asistente','admin','maestra') OR parent_id = auth.uid());

-- Pol?ticas para posts
DROP POLICY IF EXISTS "posts_select" ON public.posts;
CREATE POLICY "posts_select" ON public.posts FOR SELECT USING (
  auth.uid() IS NOT NULL AND (
    COALESCE(get_my_role(), '') IN ('directora','asistente','admin','maestra') OR
...
CREATE POLICY "posts_insert" ON public.posts FOR INSERT
  WITH CHECK (
    auth.uid() = teacher_id
    AND COALESCE(get_my_role(), '') IN ('directora','asistente','maestra','admin')
  );

DROP POLICY IF EXISTS "posts_update" ON public.posts;
CREATE POLICY "posts_update" ON public.posts FOR UPDATE
  USING (auth.uid() = teacher_id AND COALESCE(get_my_role(), '') IN ('directora','asistente','maestra','admin'));
...
CREATE POLICY "comments_insert" ON public.comments FOR INSERT
  WITH CHECK (auth.uid() = user_id AND EXISTS (
    SELECT 1 FROM public.posts p WHERE p.id = comments.post_id AND (
      COALESCE(get_my_role(), '') IN ('directora','asistente','maestra','admin') OR
      p.classroom_id IS NULL OR
      is_parent_of_classroom(p.classroom_id)
    )
  ));
...
CREATE POLICY "classroom_gallery_all" ON public.classroom_gallery FOR ALL
  USING (COALESCE(get_my_role(), '') IN ('directora','asistente','admin','maestra') OR is_parent_of_classroom(classroom_id));
...
CREATE POLICY "grades_staff" ON public.grades FOR ALL
  USING (COALESCE(get_my_role(), '') IN ('directora','asistente','maestra','admin'))
  WITH CHECK (
    COALESCE(get_my_role(), '') IN ('directora','asistente','maestra','admin') AND (
...
END $$;
-- ============================================================
-- Migración: Planes de Pago Flexibles, Facturación Fiscal y Tienda Virtual
-- ============================================================
...
-- Función para calcular subtotal de item
...
-- ============================================================
-- ----------------------------------------------------------------------
-- DATOS INICIALES — Año Escolar 2026-2027
...
-- Función temporal para insertar planes y cuotas de forma limpia
DO $$
DECLARE
  v_sy_id bigint;
  v_plan_id bigint;
